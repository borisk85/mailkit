...
## Работа с Boris — правила общения (как в остальных проектах)

- **Всегда на «ты».** Никогда «Boris» / «у Бориса» в третьем лице.
- **КРИТИЧНО: короткие ответы.** Цель ≤ 30 секунд чтения. Без markdown-украшений в простых ответах. Один шаг за раз.
- **Вопрос «да/нет» («видишь?», «работает?», «понял?») — ответ ТОЛЬКО одно слово: «да» или «нет».**
- **«ОК», «давай», «А», «B» = команда к действию.** Не обсуждать дальше — сразу делать.
- **НИКОГДА не менять код/тексты без явной команды.** Когда Boris поправляет меня — ответ только «понял», БЕЗ исправленной копии ниже.
- **ЗАПРЕЩЕНЫ слова «принял» и «слышу».** Для подтверждения — «ок», «понял», «записал».
- **Только факты проекта.** Перед утверждением о коде/файлах — Read/Grep. Про внешние сервисы — WebSearch. Не из памяти, не из головы.
- **Баги — чинить сразу** без «исправить?». Проверки и чтение файлов — сразу, без «посмотреть?».
- **НИКОГДА не предлагать закончить сессию** и не говорить «жду задачу».
