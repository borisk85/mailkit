...
**Спрос:** вероятен, но НЕ доказан как факт. 550K+ пользователей CF Email Routing (цифра 2022) настроили входящие; **гипотеза** — что значимая доля не настроила исходящие и осталась застрявшей. Размер этой доли — открытый вопрос (см. Открытый вопрос №3), верификацию не прошёл. Дополнительная оговорка: пользователи CF DNS скошены в техническую сторону, а технический человек чинит отправку бесплатно по гайду — поэтому реальный ICP это не «все 550K», а узкая прослойка тех, кто упёрся именно в спам-проблему и не смог решить сам. Размер прослойки = ключевое неизвестное.

**Деньги:** $5 one-time — входной билет, не бизнес. Возможные пути расширения — **все непроверенные гипотезы, НЕ закладывать в модель как факт:**
1. Мониторинг $3/мес (recurring) — ⚠ две непроверенные посылки: (а) захочет ли покупатель разовой настройки за $5 платить ещё и ежемесячно — спросом не подтверждено; (б) сам мониторинг доставляемости/DMARC — конкурентная ниша (Valimail, dmarcian, EasyDMARC, инструменты самого Postmark, часто с free-тиром), то есть recurring-слой может оказаться таким же коммодити, как базовый сетап.
2. Agency / «сделаю под ключ» тир — выше чек, меньше коммодитизации; вероятно честнее как апсайд, чем $3/мес мониторинг. ⚠ конфликтует с текущим scope проекта («не делать done-for-you сервисы» в CLAUDE.md) — решение за фаундером.
3. Расширение на не-CF DNS через ImprovMX API (TAM ×2-3) — тоже гипотеза, без данных спроса.
