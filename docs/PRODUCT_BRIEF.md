...
## Локализация (updated 2026-04-30)

**Только английский язык.** Русская версия удалена после анализа Cloudflare
audience: 50%+ англоговорящие, 2.5% русскоговорящие, потенциальная RU
выручка 100-500 долларов в год не оправдывает накладные расходы на
двуязычный копи для micro-SaaS масштаба.

Telegram канал и Habr longreads на русском языке (под управлением owner)
остаются как marketing channels для русскоязычной indie hacker аудитории,
с английских лендингов могут быть ссылки на эти каналы для русскоязычных
посетителей.

## Категория продукта — критично (2026-04-30)

**MailKit это email setup automation tool, НЕ email marketing platform.**

Это категориальное различие важно для позиционирования и снижения
trademark collision с Mailkit s.r.o. (чешская email marketing platform
основана 2006). Они защищены в категории email-маркетинга, мы работаем
в категории setup-автоматизации — разные продукты, разные ICP.

Везде в копи, маркетинге, SEO избегаем терминов: email marketing platform,
email service provider, ESP, transactional email service, email
infrastructure platform. Используем: email setup automation, domain
email setup tool, Cloudflare Email Routing automation, Gmail Send-As
setup wizard. Полный список — docs/MARKETING_ANGLES.md.
...
Email Routing + Postmark SMTP + DKIM/SPF/DMARC + Gmail Send-As.
...
4. Setup pipeline: Cloudflare Email Routing + Postmark sender domain автомат
...
- Postmark sender domain + DKIM + SMTP creds — полностью автоматом через API (per-tenant servers)
- Gmail Send-As — guided UI-wizard: юзер копирует SMTP host/port/username/password в Gmail Settings (deep link), получает verification email, кликает ссылку
...
| 4 | Ждет прогресс-бар | CF: Email Routing + MX/SPF/DMARC + routing rule<br>Postmark: sender domain + DKIM + SMTP creds<br>DKIM back to CF DNS<br>Postmark verification polling | ~90 sec |
...
### Postmark — **per-tenant servers** (Model C)
**Решение:** каждый customer получает изолированный Postmark server через Postmark API.
- Каждый tenant — отдельный Postmark Server с собственной репутацией и SMTP credentials
- **User-flow:** юзер **не знает** что Postmark существует, 0 шагов с его стороны
- **Cost:** Postmark pay-per-use (~$1.25/1K outbound messages)
- **Risks:** один заспамивший tenant не влияет на других — репутация изолирована per-server.
  Mitigation: rate-limits на нашей стороне, abuse detection, per-tenant CloudWatch-аналог мониторинг

### Transactional email (welcome, receipt, setup-done)
Через Postmark transactional API (тот же стек). Один сервис вместо двух, меньше env-переменных.
...
| SMTP provider | Postmark API (per-tenant servers) |
| Transactional email | Postmark transactional API (одно место) |
...
| Scale (500+ users) | ~$65 | Vercel Pro + Supabase Pro + Postmark usage |
...
1. DKIM ротация в Postmark (раз в 6-12 мес)
2. Юзер сам сломал DNS при других изменениях
3. Postmark деактивировал sender domain (inactivity / ToS)
4. Gmail отозвал Send-As (редко)
5. Истек домен / смена регистратора
6. Postmark sending limits исчерпаны
...
- Автопроверка: MX, SPF, DKIM, DMARC, Send-As status, Postmark domain status
