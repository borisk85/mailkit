...
ценность (мы автоматизируем CF + Postmark + ведём через Gmail) исчезает.
...
интегрироваться с третьим сервисом (как мы с Postmark). Для них это далеко
от core competency.
...
напрямую на Postmark (`smtp.postmarkapp.com:587`). При смене backend нужны
...
2. **Свой SMTP proxy** (`smtp.getmailkit.com` → routes to Postmark/SES).
...
3. **Migration assistance как paid service** ($10-20) при закрытии MailKit
   или их собственном решении мигрировать на свой SMTP. Документация +
