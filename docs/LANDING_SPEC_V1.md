...
>
> - Your Cloudflare API token is stored encrypted at rest only while
>   your setup is in progress (so you can resume on any device), then
>   deleted as soon as it finishes — never kept in plain text.
...
6. **"Is my data safe?"** — "Your Cloudflare token is stored encrypted
   only while your setup runs and deleted when it finishes. Your Gmail SMTP password is generated
