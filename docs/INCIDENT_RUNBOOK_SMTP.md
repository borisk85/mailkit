...
1. Mass email через работающий канал (secondary region SES либо secondary
   ...
   - Subject: "Action required: update your MailKit SMTP credentials" /
     "Требуется действие: обнови SMTP-доступы MailKit"
...
- [ ] Email templates EN/RU для migration notice + 4 follow-up'а.
...
поверх `feat/ses-backend-swap`) от 2026-04-29. Implemented в
`messages/{en,ru}.json` под ключом `terms.smtpDependency`,
...
- После каждого реального incident'а.
- Раз в 6 мес минимум.
- Перед каждым крупным релизом который меняет SMTP backend.
